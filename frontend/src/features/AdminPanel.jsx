...
export default function AdminPanel() {
  ...
  const [approving, setApproving] = useState(null); // Pour le chargement pendant l'approbation
  const dispatch = useDispatch();
  const navigate = useNavigate();

  // Récupération des infos utilisateur depuis Redux pour le token si besoin
  ...
  const handleApprove = async (id) => {
    try {
      setApproving(id); // Afficher un état de chargement
      ...
      // Mettre à jour le state Redux si c'est moi l'utilisateur approuvé
      if (userInfo && id === userInfo.id) {
        dispatch(updateUserStatus({ id, isApproved: true }));
      }
      
      fetchUsers(); // Actualiser la liste après approbation
      setApproving(null);
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center border-b border-[#9E6B6B]/30 pb-6">
          <div>
            ...
            <h1 className="text-3xl font-serif font-light">Tableau de bord Admin</h1>
          </div>
        ...
        {!loading && !error && users.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-[#FCFAFA] text-neutral-900 border border-[#9E6B6B]/30 p-4 rounded-none">
              <p className="text-xs font-mono uppercase tracking-widest text-neutral-600">Total Utilisateurs</p>
              <p className="text-3xl font-serif text-[#9E6B6B]">{users.length}</p>
            </div>
            <div className="bg-[#FCFAFA] text-neutral-900 border border-[#9E6B6B]/30 p-4 rounded-none">
              <p className="text-xs font-mono uppercase tracking-widest text-neutral-600">En attente</p>
              <p className="text-3xl font-serif text-amber-700">
                {users.filter(u => !u.isApproved && u.role !== 'admin').length}
              </p>
            </div>
            <div className="bg-[#FCFAFA] text-neutral-900 border border-[#9E6B6B]/30 p-4 rounded-none">
              <p className="text-xs font-mono uppercase tracking-widest text-neutral-600">Approuvés</p>
        ...
        {loading ? (
          <p className="text-center text-neutral-400 font-mono text-xs">Chargement des utilisateurs...</p>
        ) : error ? (
          <p className="text-center text-red-400 font-mono text-xs">{error}</p>
        ) : users.length === 0 ? (
          <p className="text-center text-neutral-400 font-mono text-xs">Aucun utilisateur inscrit.</p>
        ) : (
          <div className="bg-[#FCFAFA] text-neutral-900 border border-[#9E6B6B]/30 p-6 shadow-2xl">
            <h2 className="text-xl font-serif mb-4 flex items-center gap-2">
              <Users size={20} className="text-[#9E6B6B]" /> Liste des utilisateurs inscrits
            </h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-[#9E6B6B]/10 text-sm">
                  {users.map((u) => (
                    <tr key={u._id} className="hover:bg-neutral-100 transition-colors">
                      <td className="py-3 px-4 font-medium">{u.nom || 'N/A'}</td>
                      <td className="py-3 px-4 text-neutral-600">{u.email}</td>
                      <td className="py-3 px-4 font-mono text-xs">
                        <span className={`px-2 py-1 ${u.role === 'admin' ? 'bg-[#9E6B6B] text-white' : 'bg-neutral-200 text-neutral-800'}`}>
                          {u.role || 'user'}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4 text-center">
                        {!u.isApproved && u.role !== 'admin' && (
                          ...
                          >
                            {approving === u._id ? 'Traitement...' : 'Approuver'}
                          </button>
                        )}
                      </td>
                    </tr>
